...
- [Python](https://www.python.org/)・[Pillow](https://python-pillow.org/)：素材生成・ビルド・撮影結果のGIF化に使用。実行環境は同梱していません。

背景・結晶画像・動きのテーブルは `tools/generate.py` で本デモ用に生成したものです。配布ROMは本プロジェクトのアセンブリーと生成データから構成され、BIOS、システムROM、エミュレーター、SDCC等のランタイムは含みません。
