"""Build the two explicit d884 emulator port profiles. No BIOS required."""
...
def build():
    ...
    for profile,port in [('legacy-openmsx-internal',0x98),('legacy-openmsx',0x88)]:
        ...
        rom=boot+runtime+bytes([255])*(24576-len(runtime))+(ROOT/'assets/vram-upload.bin').read_bytes()+(ROOT/'assets/motion.bin').read_bytes()
        assert len(rom)==393216
        rom+=bytes([255])*(524288-len(rom))
        name=f'LUMEN_FORGE-V9968-{profile}.rom'
        (out/name).write_bytes(rom)
        reports.append({'file':name,'mapper':'ASCII8','rom_bytes':len(rom),'runtime_bytes':len(runtime),
                        'sha256':hashlib.sha256(rom).hexdigest(),'profile':profile,'hardware_verified':False})
    (out/'build-manifest.json').write_text(json.dumps(reports,indent=2)+'\n')
