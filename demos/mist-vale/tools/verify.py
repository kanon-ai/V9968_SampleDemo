...
def execute(profile, capture=False):
    ...
    vb=int(re.search(r'vblank_counts\s+EQU 0([0-9A-F]+)H',sym)[1],16)
    rc=int(re.search(r'raster_counts\s+EQU 0([0-9A-F]+)H',sym)[1],16)
...
proc tick {} {
  if {[catch {
    ...
    lappend ::prism_samples [list $::prism_count $frame [debug read {S1990 regs} 6] [debug read {REGS} 20] [debug read {REGS} 2] [expr {[debug read memory VB]+256*[debug read memory VBN]}] [expr {[debug read memory RC]+256*[debug read memory RCN]}]]
    if {CAPTURE && $::prism_count==72} {
      record start ../capture/MIST_VALE-native.avi
    }
    if {CAPTURE && $::prism_count==216} {
      record stop
      openmsx::internal_screenshot -raw ../../outputs/MIST_VALE-emulator.png
    }
    incr ::prism_count
    if {$::prism_count<576} {after time 0.083333333333 tick} else {
      ...
      puts -nonewline $f [debug read_block {VRAM} 131072 131072]; close $f
      set f [open attrs-PROFILE.bin wb]
      puts -nonewline $f [debug read_block {VRAM} 28672 104]; close $f
      exit
    }
...
'''
    for a,b in {'THROTTLE':'true' if capture else 'false','VIDEO_SELECT':'' if internal else 'after time 1 {set videosource V9968}','ADDRESS':str(addr),'NEXT':str(addr+1),
                'VBN':str(vb+1),'VB':str(vb),'RCN':str(rc+1),'RC':str(rc),'REGS':regs,'VRAM':vram,'CAPTURE':str(int(capture)),'PROFILE':profile}.items():
    ...
    # SCREEN8 planar display selection uses R2 bit 5 for logical Y=0/256.
    # 3F/7F both select Y=256; merely observing two values misses that alias.
    assert {(row[4] >> 5) & 1 for row in stable} == {0,1}, 'Physical display pages did not alternate'
    ...
    assert attrs[96:]==bytes([216,0,0,0,0,0,0,0])
    assert {attrs[8*n+3]>>6 for n in range(12)}=={3}
    vb_delta=(stable[-1][5]-stable[0][5])%65536
    raster_delta=(stable[-1][6]-stable[0][6])%65536
    assert 10.8<raster_delta/vb_delta<11.2
    rom=OUT/f'MIST_VALE-V9968-{profile}.rom'
    return {'profile':profile,'sha256':hashlib.sha256(rom.read_bytes()).hexdigest(),
        ...
        'loop_wrap_seen':True,'r800_dram':True,'asset_upload_byte_exact':True,'twelve_sprites_plus_terminator':True,'vblank_interrupts':vb_delta,'raster_interrupts':raster_delta,'raster_events_per_vblank':round(raster_delta/vb_delta,3),
        'transparency_values':[75],
...
if __name__=='__main__':
    ...
    assert (CAP/'MIST_VALE-native.avi').stat().st_size>1024
    report={'emulator_sha256':hashlib.sha256(EXE.read_bytes()).hexdigest(),'verified':results,
        'native_recording_sha256':hashlib.sha256((CAP/'MIST_VALE-native.avi').read_bytes()).hexdigest(),
        'native_recording_rom_sha256':results[0]['sha256'],
