...
**2026-09-22 / openMSX 194a769:** ROMは従来のまま、[V9968_OLD互換マシン設定](../../UPDATE-20260922.md)を使用してください。最新版で65秒、約59.92更新／秒、ループとページ切替を再確認しました。通常の `V9968` 設定では背景が崩れます。 / Keep this ROM and use the linked V9968_OLD compatibility XML on 194a769.
...
**[技術解説：座標変換・仮想光源・船上の猫・メモリー配置 / Technical explanation](TECHNIQUES.md)**

## 見どころと実装

- SCREEN5の256×212画面全域を、V9968のLRMMで毎更新変形。256×768の地形原画から回転・拡縮・移動を同時に描画します。
- 航路の接線方向に画面を向け、旋回中も常に前進します。約34.18秒で拡張した海岸を2周し、高度は独立に変化します。
- 地形倍率は約1.50〜4.5倍。猫の傾き、背中に重なる短いマント、影の大きさと距離、半透明の雲を組み合わせています。
- 光源を地形の北西に固定し、旋回時には地上の影の方向も地形と一緒に回ります。
- 地形を従来の1.5倍へ拡張。3隻の屋形船に計6匹の猫がいて、手を振ったり踊ったりします。
...
36秒のエミュレーター実行で2159回、約59.92回／秒の更新を確認しました。全2158更新間隔は1映像フレームで、ループ境界、原画と船上アニメーション全8段階のVRAM一致、R800 DRAM、コマンド終了後のVBlank表示も検証しています。`outputs/verification.json`、`motion-verification.json`、`video-verification.json`、`reproducibility.json`を参照してください。これらは実機の性能保証ではありません。
...
The 512 KiB ASCII8 ROM uses a live 256×212 LRMM transform and up to ten Sprite mode3 entries. Terrain is a source image with six small animated deck patches; the ROM stores motion parameters rather than completed video frames. A 2048-update loop follows two longer forward circuits with independent altitude changes. The terrain is 1.5 times larger than the initial version, with six waving/dancing cats on three boats. Shadow displacement follows a fixed world-space light, so its screen direction rotates naturally with the landscape.
